...
window.addEventListener('DOMContentLoaded', function () {
  ...
  var modalState = {}; // состояние модального окна, где пользователь что-то выбирает

  var deadline = '2021-12-18';
...
var changeModalState = function changeModalState(state) {
  ...
  Object(_checkNumInputs__WEBPACK_IMPORTED_MODULE_1__["default"])('#height'); // запись только цифр
...
var forms = function forms(state) {
  ...
  Object(_checkNumInputs__WEBPACK_IMPORTED_MODULE_5__["default"])('input[name="user_phone"]'); // запись только цифр
  ...
  form.forEach(function (item) {
    item.addEventListener('submit', function (e) {
      ...
      item.appendChild(statusMessage); // создали блок с сообщением о статусе

      var formData = new FormData(item); // сбор данных из формы
...
var images = function images() {
  ...
  workSection.addEventListener('click', function (e) {
    ...
    if (target && target.classList.contains('preview')) {
      ...
      document.body.style.overflow = 'hidden'; // показываем модалку и запрещаем прокрутку body

      var path = target.parentNode.getAttribute('href');
      bigImg.setAttribute('src', path); // получаем большую картинку по ссылке родителя эл-та, на который кликнули
    }

    if (target && target.matches('div.popup')) {
      imgPopup.style.display = 'none';
      document.body.style.overflow = ''; // закрываем модалку и возвращаем прокрутку body
    }
...
var tabs = function tabs(headerSelector, tabSelector, contentSelector, activeClass) {
  ...
  header.addEventListener('click', function (e) {
    ...
    if (target && (target.classList.contains(tabSelector.replace(/\./, '')) || target.parentNode.classList.contains(tabSelector.replace(/\./, '')))) {
      // проверка клика именно на tab, убираем точку у класса
